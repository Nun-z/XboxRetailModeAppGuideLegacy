...
**3.2.3.**  Right-click your .appx file and open it with WinRar.  There are many sources of RetroArch packages that will run on an Xbox.  The official ones are here, listed by version number: https://buildbot.libretro.com/stable/.  Bundle files will need to be opened with WinRar in order to extract the .msix or .appx file inside it.  You can rename a .msix file to give it a .appx file extension, and it should still work.
